...
public final class StructurePieceCountsAdditionsMerger {
    ...
    static void performCountsAdditionsDetectionAndMerger(ResourceManager resourceManager) {
        Map<ResourceLocation, JsonElement> countsAdditionJSON = GeneralUtils.getDatapacksJSONElement(resourceManager, GSON, DATA_TYPE, FILE_SUFFIX_LENGTH);
     ...
     */
    private static void parseCountsAndBeginMerger(Map<ResourceLocation, JsonElement> countsAdditionJSON) {
        for (Map.Entry<ResourceLocation, JsonElement> entry : countsAdditionJSON.entrySet()) {
